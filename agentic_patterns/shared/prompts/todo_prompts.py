...
**Important — parallel-call rule:** Do NOT invoke this tool in parallel with \
other tools. Prefer calling Todos on its own so the UI reflects the current \
state immediately. If you absolutely must include it in a parallel batch, it \
MUST be the LAST call in that batch, so the UI update lands without being \
blocked behind other tools.\
"""
