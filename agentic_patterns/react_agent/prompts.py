...
# Tools
- Prefer dedicated tools over `bash` when one fits (Read, Write, \
    Edit, Grep, Glob) — reserve `bash` for shell-only work \
...
- Before calling a tool, confirm it's in your registered tool set. If a task \
    ...
    user to run it.
- Use `Todos` for multi-step work. Flip an item to `in_progress` right \
...
# Tool combos
- Unknown location → Glob to find the file, then Read.
- Known content, unknown line → Grep (content mode) → note the line number → \
    Read with offset=<line> for a precise window.
- Before Edit → Read the target region first so old_string matches exactly.
- Large files (> ~20k tokens) → Grep to pinpoint, then windowed Read.
